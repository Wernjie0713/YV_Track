<!--profile-->
<div class="row">
    <?php
        $data['activities'];
        $index=0;
        $totalparticipants=0;
        $upcomingactivities=0;
        if($data['activities'])
        {
            foreach($data['activities'] as $activity):
                $index++;
                $totalparticipants=$totalparticipants+$activity->no_participants;
            endforeach;
            foreach($data['upcomingActivities'] as $activity):
                $upcomingactivities++;
            endforeach; 
    }?>
  <div class="col-sm-3">
    <div class="card" style="width:22rem; height:10rem">
      <div class="card-body">
      <img src="data:image/png;base64," style="width: 35px; margin-bottom:10px;">
      <h4 class="card-title" style="margin-left:5px"><?php echo $index ?></h5>
      <p class="card-text" style="margin-left:5px;font-size: 10px; color:gray">Total Number of published activities </p>
      </div>
    </div>
  </div>
  <div class="col-sm-3">
    <div class="card" style="width:22rem; height:10rem">
      <div class="card-body">
        <img src="data:image/png;base64," style="width: 35px; margin-bottom:8px;">
        <h4 class="card-title" style="margin-left:5px"><?php echo $totalparticipants ?></h4>
        <p class="card-text" style="margin-left:5px;font-size: 10px; color:gray">Total Number of participants </p>
      </div>
    </div>
  </div>
  <div class="col-sm-3">
    <div class="card" style="width:22rem; height:10rem">
      <div class="card-body">
        <img src="data:image/png;base64," style="width: 35px; margin-bottom:10px;">
        <h4 class="card-title" style="margin-left:5px"><?php echo $upcomingactivities ?></h4>
        <p class="card-text" style="margin-left:5px;font-size: 10px; color:gray">Number of Upcoming Activities</p>
      </div>
    </div>
  </div>
</div>

<!--end of profile-->

<br>

<!--start list of student-->
<div class="card shadow-sm">
    <div class="card-header">
        <h3 class="card-title">Student List</h3>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table id="kt_datatable_student" class="table table-row-bordered gy-5">
            <thead>
                    <tr class="fw-semibold fs-6 text-muted">
                        <th style="width: 5%;">#</th>           
                        <th style="width: 10%;">Profile Picture</th>
                        <th style="width: 15%;">Name</th>
                        <th style="width: 5%;">Age</th>
                        <th style="width: 10%;">Contact</th>
                        <th style="width: 10%;">Email</th>
                        <th style="width: 15%;">University</th>
                        <th style="width: 20%;">Course</th>
                    </tr>
                </thead>
                <tbody>
                    <?php $index =1;
                    foreach($data['pages'] as $student): ?>
                    <tr>
                        <td style="width: 5%;"><?php echo $index . "."; ?></td>
                        <td style="width: 10%;"><?php if(isset($student->stu_photo) && !empty($student->stu_photo)):?>
                                                <div class="image-input" data-kt-image-input="true" style="background-image: url('<?php echo URLROOT."/public/".$student->stu_photo; ?>'); width: 40px; height: 40px; display: flex; justify-content: center; align-items: center;"> <!-- Removed image-input-outline class -->
                                                    <div class="image-input-wrapper w-40px h-40px rounded-circle" style="background-image: url('<?php echo URLROOT."/public/".$student->stu_photo; ?>');"></div>
                                                </div>
                                            <?php else:?>
                                                <div class="custom-img" style="width: 40px; height: 40px; overflow: hidden; border-radius: 50%; margin-top: 20px; margin-left: 3px; display: flex; justify-content: center; align-items: center;">
                                                    <img src="<?php echo URLROOT . "/public/assets/media/youth/default_stupfp.png" ?>" >
                                                </div>
                                            <?php endif; ?></td>
                        <td style="width: 15%;"><a href="<?php echo URLROOT; ?>/pages/profileview/<?php echo $student->stu_email; ?>" class="link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover"><?php echo $student->stu_name; ?></a></td>
                        <td style="width: 5%;"><?php echo $student->stu_age; ?></td>
                        <td style="width: 10%;"><?php echo $student->stu_phone; ?></td>
                        <td style="width: 10%;"><?php echo $student->stu_email; ?></td>
                        <td style="width: 15%;"><?php echo $student->stu_university; ?></td>
                        <td style="width: 20%;"><?php echo $student->stu_course; ?></td>
                        <?php $index++; ?>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        </div>
        <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>



        <script>
        $(document).ready(function() {
            var table = $('#kt_datatable_student').DataTable({

            });
        });
        </script>

    </div>
</div>
<!--end list of student-->

<br>

<!--start list of client-->
<div class="card shadow-sm">
    <div class="card-header">
        <h3 class="card-title">Client List</h3>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table id="kt_datatable_client" class="table table-row-bordered gy-5">
            <thead>
                    <tr class="fw-semibold fs-6 text-muted">
                        <th style="width: 5%;">#</th>
                        <th style="width: 10%;">Profile Picture</th>
                        <th style="width: 15%;">Organization</th>   
                        <th style="width: 10%;">Contact</th>
                        <th style="width: 15%;">Email</th>
                    </tr>
                </thead>
                <tbody>
                    <?php $index =1;
                    foreach($data['clientlist'] as $client): ?>
                    <tr>
                        <td style="width: 5%;"><?php echo $index . "."; ?></td>
                        <td style="width: 10%;"><?php if(isset($client->client_photo) && !empty($client->client_photo)):?>
                                    <div class="image-input" data-kt-image-input="true" style="background-image: url('<?php echo URLROOT."/public/".$client->client_photo; ?>'); width: 40px; height: 40px; display: flex; justify-content: center; align-items: center;"> <!-- Removed image-input-outline class -->
                                        <div class="image-input-wrapper w-40px h-40px rounded-circle" style="background-image: url('<?php echo URLROOT."/public/".$client->client_photo; ?>');"></div>
                                    </div>
                                    <?php else:?>
                                    <div class="custom-img" style="width: 40px; height: 40px; overflow: hidden; border-radius: 50%; margin-top: 20px; margin-left: 3px; display: flex; justify-content: center; align-items: center;">
                                        <img src="<?php echo URLROOT . "/public/assets/media/youth/default_cpfp.png" ?>" >
                                    </div>
                        <?php endif; ?></td>
                        <td style="width: 15%;"><a href="<?php echo URLROOT; ?>/dashboards/cprofileview/<?php echo  $client->client_email; ?>" class="link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover"><?php echo $client->client_organization; ?></a></td>
                        <td style="width: 10%;"><?php echo $client->client_phone; ?></td>
                        <td style="width: 15%;"><?php echo $client->client_email; ?></td>
                        
                        <?php $index++; ?>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        </div>
        <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>



        <script>
        $(document).ready(function() {
            var table = $('#kt_datatable_client').DataTable({

            });
        });
        </script>

    </div>
</div>
<!--end list of client-->

<br>

<!--start Top 10 Most Attended Activities-->
<div class="card shadow-sm">
    <div class="card-header">
        <h3 class="card-title">Top 10 Most Attended Activities</h3>
    </div>
    <div class="card-body">
       
        <div class="table-responsive">
            <table style="width:100%" id="kt_datatable_activity" class="table table-row-bordered gy-5">
                <thead>
                    <tr class="fw-semibold fs-6 text-muted">
                        <th style="width: 5%;">#</th>
                        <th style="width: 15%;">Activity</th>
                        <th style="width: 20%;">Description</th>
                        <th style="width: 15%;">Date</th>
                        <th style="width: 5%;">Duration</th>
                        <th style="width: 5%;">Given Marks</th>
                        <th style="width: 5%;">No. Participants</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if($data['activities']){
                    $index=1;
                    foreach($data['activities'] as $activity):  ?>
                    <tr>
                        <?php if($index<=10) {?>
                        <td style="width: 5%;"><?php echo $index++ . "."; ?></td>
                        <td style="width: 15%;"><?php echo $activity->act_name; ?></td>
                        <td style="width: 20%;"><?php echo $activity->act_des; ?></td>
                        <td style="width: 15%;"><?php echo date('d/m/y ', strtotime($activity->act_start)); ?> - <?php echo date('d/m/y ', strtotime($activity->act_end)); ?></td>
                        <td style="width: 5%;"><?php echo $activity->act_duration; ?></td>
                        <td style="width: 5%;"><?php echo $activity->act_mark; ?></td>
                        <td style="width: 5%;"><?php echo $activity->no_participants; ?></td>
                        <?php } ?>
                    </tr>
                    <?php endforeach; }
                    else ?>
                    <tr><td colspan="5"><?php echo "No recorded activities.";?></td></tr>

                </tbody>
            </table>
        </div>

        <div class="card-footer">
            <div style="text-align: right;">
                <a class="link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover" href="<?php echo URLROOT; ?>/activities">
                    To see all published activities->
                </a>
            </div>
        </div>
    </div>
</div>
<!--end Top 10 Most Attended Activities-->-